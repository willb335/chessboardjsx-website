...
export const rough = (function() {
    ...
    processPoints() {
      ...
      for (let i = 0; i < this.segments.length; i++) {
        ...
        switch (s.key) {
          ...
          case 't':
            s.point = [
              s.data[0] + currentPoint[0],
              s.data[1] + currentPoint[1]
            ];
            break;
          case 'H':
          ...
          case 'c':
            s.point = [
              s.data[4] + currentPoint[0],
              s.data[5] + currentPoint[1]
            ];
            break;
          case 'S':
          ...
          case 's':
            s.point = [
              s.data[2] + currentPoint[0],
              s.data[3] + currentPoint[1]
            ];
            break;
          case 'Q':
            s.point = [s.data[2], s.data[3]];
          ...
          case 'a':
            s.point = [
              s.data[5] + currentPoint[0],
              s.data[6] + currentPoint[1]
            ];
            break;
        }
        if (s.key === 'm' || s.key === 'M') {
  ...
  class RoughArcConverter {
    constructor(from, to, radii, angle, largeArcFlag, sweepFlag) {
      ...
      const x1dash =
        (this._cosPhi * (from[0] - to[0])) / 2.0 +
        (this._sinPhi * (from[1] - to[1])) / 2.0;
      const y1dash =
        (-this._sinPhi * (from[0] - to[0])) / 2.0 +
        (this._cosPhi * (from[1] - to[1])) / 2.0;
      let root = 0;
      const numerator =
        this._rx * this._rx * this._ry * this._ry -
        this._rx * this._rx * y1dash * y1dash -
        this._ry * this._ry * x1dash * x1dash;
      if (numerator < 0) {
        const s = Math.sqrt(
          1 - numerator / (this._rx * this._rx * this._ry * this._ry)
        );
        this._rx = this._rx * s;
        this._ry = this._ry * s;
        root = 0;
      ...
      this._C = [0, 0];
      this._C[0] =
        this._cosPhi * cxdash - this._sinPhi * cydash + (from[0] + to[0]) / 2.0;
      this._C[1] =
        this._sinPhi * cxdash + this._cosPhi * cydash + (from[1] + to[1]) / 2.0;
      this._theta = this.calculateVectorAngle(
        1.0,
        0.0,
        (x1dash - cxdash) / this._rx,
        (y1dash - cydash) / this._ry
      );
      let dtheta = this.calculateVectorAngle(
        (x1dash - cxdash) / this._rx,
        (y1dash - cydash) / this._ry,
        (-x1dash - cxdash) / this._rx,
        (-y1dash - cydash) / this._ry
    ...
    getNextSegment() {
      ...
      const to = [
        this._cosPhi * this._rx * cosTheta2 -
          this._sinPhi * this._ry * sinTheta2 +
          this._C[0],
        this._sinPhi * this._rx * cosTheta2 +
          this._cosPhi * this._ry * sinTheta2 +
          this._C[1]
      ];
      const cp1 = [
        this._from[0] +
          this._T *
            (-this._cosPhi * this._rx * sinTheta1 -
              this._sinPhi * this._ry * cosTheta1),
        this._from[1] +
          this._T *
            (-this._sinPhi * this._rx * sinTheta1 +
              this._cosPhi * this._ry * cosTheta1)
      ];
      const cp2 = [
        to[0] +
          this._T *
            (this._cosPhi * this._rx * sinTheta2 +
              this._sinPhi * this._ry * cosTheta2),
        to[1] +
          this._T *
            (this._sinPhi * this._rx * sinTheta2 -
              this._cosPhi * this._ry * cosTheta2)
      ];
      this._theta = theta2;
    ...
    intersects(otherSegment) {
      ...
      if (grad1 === Number.MAX_VALUE) {
        if (grad2 === Number.MAX_VALUE) {
          ...
          if (
            this.py1 >= Math.min(otherSegment.py1, otherSegment.py2) &&
            this.py1 <= Math.max(otherSegment.py1, otherSegment.py2)
          ) {
            this.xi = this.px1;
            this.yi = this.py1;
            return true;
          ...
          if (
            this.py2 >= Math.min(otherSegment.py1, otherSegment.py2) &&
            this.py2 <= Math.max(otherSegment.py1, otherSegment.py2)
          ) {
            this.xi = this.px2;
            this.yi = this.py2;
            return true;
        ...
        if (
          (this.py1 - this.yi) * (this.yi - this.py2) < -0.00001 ||
          (otherSegment.py1 - this.yi) * (this.yi - otherSegment.py2) < -0.00001
        ) {
          return false;
        }
        if (Math.abs(otherSegment.a) < 0.00001) {
          if (
            (otherSegment.px1 - this.xi) * (this.xi - otherSegment.px2) <
            -0.00001
          ) {
            return false;
          }
      ...
      if (grad2 === Number.MAX_VALUE) {
        ...
        if (
          (otherSegment.py1 - this.yi) * (this.yi - otherSegment.py2) <
            -0.00001 ||
          (this.py1 - this.yi) * (this.yi - this.py2) < -0.00001
        ) {
          return false;
      ...
      if (grad1 === grad2) {
        ...
        if (
          this.px1 >= Math.min(otherSegment.px1, otherSegment.px2) &&
          this.px1 <= Math.max(otherSegment.py1, otherSegment.py2)
        ) {
          this.xi = this.px1;
          this.yi = this.py1;
          return true;
        ...
        if (
          this.px2 >= Math.min(otherSegment.px1, otherSegment.px2) &&
          this.px2 <= Math.max(otherSegment.px1, otherSegment.px2)
        ) {
          this.xi = this.px2;
          this.yi = this.py2;
          return true;
      ...
      this.yi = grad1 * this.xi + int1;
      if (
        (this.px1 - this.xi) * (this.xi - this.px2) < -0.00001 ||
        (otherSegment.px1 - this.xi) * (this.xi - otherSegment.px2) < -0.00001
      ) {
        return false;
      }
      return true;
  ...
  class HachureFiller {
    ...
    _fillEllipse(cx, cy, width, height, o, connectEnds = false) {
      const lines = hachureLinesForEllipse(
        cx,
        cy,
        width,
        height,
  ...
  class RoughRenderer {
    ...
    curve(points, o) {
      ...
      const o2 = this._curveWithOffset(
        points,
    ...
    ellipse(x, y, width, height, o) {
      ...
      const o1 = this._ellipse(
        ...
        ry,
        1,
        increment * this.getOffset(0.1, this.getOffset(0.4, 1, o), o),
        o
      );
        ...
        ops.push({
          ...
          data: [
            midDispX +
              x1 +
              (x2 - x1) * divergePoint +
              this.getOffset(-halfOffset, halfOffset, o),
            midDispY +
              y1 +
              (y2 - y1) * divergePoint +
              this.getOffset(-halfOffset, halfOffset, o),
        ...
        ops.push({
          ...
          data: [
            midDispX +
              x1 +
              (x2 - x1) * divergePoint +
              this.getOffset(-offset, offset, o),
            midDispY +
              y1 +
              (y2 - y1) * divergePoint +
              this.getOffset(-offset, offset, o),
      ...
      if (len > 3) {
        ...
        if (closePoint && closePoint.length === 2) {
          ...
          ops.push({
            op: 'lineTo',
            data: [
              closePoint[0] + this.getOffset(-ro, ro, o),
              closePoint[1] + +this.getOffset(-ro, ro, o)
            ]
          });
        }
      } else if (len === 3) {
      ...
      for (let i = 0; i < 2; i++) {
        if (i === 0) {
          ...
          ops.push({
            op: 'move',
            data: [
              path.x + this.getOffset(-ros[0], ros[0], o),
              path.y + this.getOffset(-ros[0], ros[0], o)
            ]
          });
        }
        f = [
          x + this.getOffset(-ros[i], ros[i], o),
          y + this.getOffset(-ros[i], ros[i], o)
        ];
        ops.push({
          op: 'bcurveTo',
          data: [
            x1 + this.getOffset(-ros[i], ros[i], o),
            y1 + this.getOffset(-ros[i], ros[i], o),
            x2 + this.getOffset(-ros[i], ros[i], o),
            y2 + this.getOffset(-ros[i], ros[i], o),
            f[0],
            f[1]
          ]
        });
    ...
    _processSegment(path, seg, prevSeg, o) {
      ...
      switch (seg.key) {
        ...
        case 'z': {
          if (path.first) {
            ops = ops.concat(
              this.doubleLine(path.x, path.y, path.first[0], path.first[1], o)
            );
        ...
        case 'q': {
          ...
          if (seg.data.length >= 4) {
            ...
            ops.push({
              op: 'move',
              data: [
                path.x + this.getOffset(-offset1, offset1, o),
                path.y + this.getOffset(-offset1, offset1, o)
              ]
            });
            let f = [
              x + this.getOffset(-offset1, offset1, o),
              y + this.getOffset(-offset1, offset1, o)
            ];
            ops.push({
              op: 'qcurveTo',
              data: [
                x1 + this.getOffset(-offset1, offset1, o),
                y1 + this.getOffset(-offset1, offset1, o),
                f[0],
                f[1]
              ]
            });
            ops.push({
              op: 'move',
              data: [
                path.x + this.getOffset(-offset2, offset2, o),
                path.y + this.getOffset(-offset2, offset2, o)
              ]
            });
            f = [
              x + this.getOffset(-offset2, offset2, o),
              y + this.getOffset(-offset2, offset2, o)
            ];
            ops.push({
              op: 'qcurveTo',
              data: [
                x1 + this.getOffset(-offset2, offset2, o),
                y1 + this.getOffset(-offset2, offset2, o),
                f[0],
                f[1]
              ]
            });
  ...
  class RoughGenerator {
    ...
    linearPath(points, options) {
      ...
      return this._drawable(
        'linearPath',
        [this.lib.linearPath(points, false, o)],
        o
      );
    }
    arc(x, y, width, height, start, stop, closed = false, options) {
      ...
      if (closed && o.fill) {
        if (o.fillStyle === 'solid') {
          ...
          paths.push(
            this.lib.patternFillArc(x, y, width, height, start, stop, o)
          );
        }
      }
      paths.push(
        this.lib.arc(x, y, width, height, start, stop, closed, true, o)
      );
      ...
      for (const item of drawing.ops) {
        ...
        switch (item.op) {
          case 'move':
            ...
            break;
          case 'bcurveTo':
            path += `C${data[0]} ${data[1]}, ${data[2]} ${data[3]}, ${
              data[4]
            } ${data[5]} `;
            break;
          case 'qcurveTo':
        ...
        switch (drawing.type) {
          ...
          case 'path2Dpattern': {
            ...
            if (doc) {
              ...
              console.error(
                'Cannot render path2Dpattern. No defs/document defined.'
              );
            }
            break;
          }
  ...
  class RoughGeneratorAsync extends RoughGenerator {
    ...
    async line(x1, y1, x2, y2, options) {
      ...
      return this._drawable(
        'line',
        [await this.lib.line(x1, y1, x2, y2, o)],
        o
      );
    }
    // @ts-ignore
    ...
    async linearPath(points, options) {
      ...
      return this._drawable(
        'linearPath',
        [await this.lib.linearPath(points, false, o)],
        o
      );
    }
    // @ts-ignore
    async arc(x, y, width, height, start, stop, closed = false, options) {
      ...
      if (closed && o.fill) {
        if (o.fillStyle === 'solid') {
          const shape = await this.lib.arc(
            x,
            y,
            width,
            height,
          ...
          paths.push(
            await this.lib.patternFillArc(x, y, width, height, start, stop, o)
          );
        }
      }
      paths.push(
        await this.lib.arc(x, y, width, height, start, stop, closed, true, o)
      );
      return this._drawable('arc', paths, o);
    }
    // @ts-ignore
  ...
  class RoughCanvasAsync extends RoughCanvas {
    ...
    // @ts-ignore
    async arc(x, y, width, height, start, stop, closed = false, options) {
      const d = await this.genAsync.arc(
        x,
        y,
        width,
        height,
  ...
  class RoughSVG {
    ...
    get defs() {
      ...
      if (doc) {
        if (!this._defs) {
          const dnode = doc.createElementNS(
          ...
          case 'path2Dpattern': {
            if (!this.defs) {
              ...
              pattern.setAttribute(
                'viewBox',
                `0 0 ${Math.round(size[0])} ${Math.round(size[1])}`
              );
